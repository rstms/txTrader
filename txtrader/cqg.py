...
"""
  ...
  TxTrader CQG API interface module
...
# 0==never shutdown, 1==shutdown after DISCONNECT_TIMEOUT seconds
SHUTDOWN_IF_NO_DATA_CONNECTION = 1
    ...
    def __str__(self):
        if self.quote:
            if self.quote.IsValid:
                ret = 'Name=%s,Type=%d,Price=%f' % (
                    self.quote.Name, self.quote.Type, self.quote.Price)
                if self.quote.HasVolume:
...
class CQG_Symbol():
    ...
    def __str__(self):
        ...
        if self.instrument:
            items.extend([self.instrument.FullName,
                          self.Bid, self.Ask, self.Trade])
    ...
    def add_client(self, client):
        self.output('CQG_Symbol %s %s adding client %s' %
                    (self, self.symbol, client))
        self.clients.add(client)

    def del_client(self, client):
        self.output('CQG_Symbol %s %s deleting client %s' %
                    (self, self.symbol, client))
    ...
    def update_quote(self):
        quote = 'quote.%s:%s %d %s %d' % (
            self.symbol, self.bid, self.bidsize, self.ask, self.asksize)
    ...
    def update_trade(self):
        if self.last_volume != self.volume:
            ...
            self.cel.WriteAllClients('trade.%s:%s %d %d' % (
...
class CQG_Catcher():
    ...
    def send_next_order(self):
        try:
            if not self.order_queue.empty():
                ...
                self.process_order(symbol, otype, _stop_price,
                                   _limit_price, quantity, callback)
        except Exception, ex:
            self.WriteAllClients('exception: %s' %
    ...
    def order_query(self):
        for Iaccount in self.Accounts:
            ...
            if query.Status == constants.rsInProgress:
                self.output('activating order query %s' %
    ...
    def request_executions(self, callback):
        self.orders_callbacks.append(
            CQG_Callback(self, 0, 'executions', callback))
        self.order_query()
    ...
    def OnQueryProgress(self, Iquery, Ierror):
        if Ierror:
            ...
            if self.IsValid(error):
                self.output('error: OrdersQuery: %s' %
                            (repr((error.Code, error.Description))))
        if Iquery:
            ...
            if query.Status == constants.rsInProgress:
                ...
                self.output('completing order query %s' %
                            account.GWAccountName)
                ...
                if not self.order_queries:
                    for cb in self.orders_callbacks:
                        if cb.label == 'orders':
                            ...
                            self.output(
                                'error: unknown order query callback type %s' % cb.label)
                    self.orders_callbacks = []
    ...
    def parse_order(self, order):
        ...
        self.output('parse_order(%s) (%d properties)' %
                    (repr(order), properties.Count))
        ret = {}
        counter = 0
        for prop in [Dispatch(p) for p in properties]:
            ...
            if not prop.Type in [constants.opTradingExpression, constants.opIsCareOrder, constants.opCareOrderIDs]:
                if prop.Type in fieldmap.keys():
                    ...
                    if prop.Value:
                        ret[prop.Name] = time.strftime(
                            '%Y-%m-%d %H:%M:%S', time.localtime(int(prop.Value)))
                else:
        ...
        for trxn in [Dispatch(t) for t in transactions]:
            if trxn.Status < len(oeventmap):
                ...
                sidx = 0
            txnstr += '%s-%s-%s' % (trxn.Id, time.strftime('%Y-%m-%d %H:%M:%S',
                                                           time.localtime(int(trxn.Timestamp))), oeventmap[sidx])
    ...
    def parse_fill(self, fill):
        ...
        ret['ServerTimestamp'] = time.strftime(
            '%Y-%m-%d %H:%M:%S', time.localtime(int(fill.ServerTimestamp)))
        ret['Timestamp'] = time.strftime(
            '%Y-%m-%d %H:%M:%S', time.localtime(int(fill.Timestamp)))
        ...
        for trade in [Dispatch(t) for t in fill.GWTrades]:
            ...
            t['ExchangeTradeDate'] = time.strftime(
                '%Y-%m-%d', time.localtime(int(trade.ExchangeTradeDate)))
            ...
            t['Side'] = trade.Side
            t['StatementDate'] = time.strftime(
                '%Y-%m-%d', time.localtime(int(trade.StatementDate)))
            ret['trades'].append(t)
        return ret
    ...
    def OnAccountChanged(self, changeType, account, position):
        if changeType == constants.actAccountChanged:
            ...
            change = 'Position Changed: %s %s' % (
                repr(account), repr(position))
    ...
    def OnTimeSeriesChanged(self, timeSeries, timeSeriesRecord, recordIndex, changeType):
        self.output('TimeSeriesChanged: %s, %s, %s, %s' % (
            repr(timeSeries), repr(timeSeriesRecord), repr(recordIndex), repr(changeType)))
    ...
    def OnOrderChanged(self, change_type, cqg_order, old_properties, cqg_fill, cqg_error):
        ...
        self.output('OrderChanged: %s' % repr(
            (change_type_string[change_type], cqg_order, old_properties, cqg_fill, cqg_error)))
        if cqg_order:
            ...
            self.WriteAllClients('order.%s: %s' %
                                 (guid, json.dumps(self.orders[guid])))

        if cqg_fill:
            fill = Dispatch(cqg_fill)
            self.WriteAllClients('execution.%s: %s' %
                                 (guid, json.dumps(self.parse_fill(fill))))

        if cqg_error:
            cqgerr = Dispatch(cqg_error)
            self.WriteAllClients('cqg_error: %s' % json.dumps(
                {'code': cqgerr.Code, 'description': repr(cqgerr.Description)}))
    ...
    def query_bars(self, bar_symbol, bar_period, bar_start, bar_end, callback):
        self.output('client: query_bars%s' %
                    repr((bar_symbol, bar_period, bar_start, bar_end)))
        ...
        try:
            ...
            # if 1==2:
            self.WriteAllClients('error: QueryBars(%s) failed!' % repr(
                (bar_symbol, bar_period, bar_start, bar_end)))
            cb.complete(None)
    ...
    def OnTimedBarsResolved(self, cqgbars, cqgerr):
        self.output('TimedBarsResolved Event: %s,%s' %
                    (repr(cqgbars), repr(cqgerr)))
        bars = []
        ...
        if not found:
            self.output('ID not found in pending bardata_callbacks id=%s, callbacks=%s' % (
                repr(ts.Id), repr([cb.Id for cb in self.bardata_callbacks])))
...
class CQG():
    def __init__(self):
        self.cel = win32com.client.DispatchWithEvents(
            'CQG.CQGCEL.4', CQG_Catcher)
